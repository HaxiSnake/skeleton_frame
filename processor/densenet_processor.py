...
class Processor(IO):
    ...
    def start(self):
        ...
        # training phase
        if self.arg.phase == 'train':
            # init pre_mena_loss by Dong BEGIN
            self.epoch_info["best_top1"]=0
            self.epoch_info["top1"]=0
            # init pre_mena_loss by Dong END
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    self.io.print_log('Done.')
                # save best model
                if (self.epoch_info['best_top1']<self.epoch_info['top1']):
                    filename = 'best_model.pt'
                    self.epoch_info['best_top1']=self.epoch_info['top1']
                    self.io.save_model(self.model,filename)
        # test phase
    ...
    def get_parser(add_help=False):
        ...
        # feeder
        parser.add_argument('--feeder', default='feeder.feeder_densenet_skating', help='data loader will be used')
